...
RSpec.describe Naborly::Ruby::Client::RentalApplications do
  ...
  let(:application_id) { '976adde2-65b4-41a2-bc07-5ad2e7a8d2da' }
  let(:attachment_id)  { '9eaaec25-17c4-4f26-998d-caa992474b99' }
  ...
  describe '#create_rental_application' do
    it 'creates a rental application' do
      VCR.use_cassette('create_rental_application') do
        naborly_client.create_rental_application(new_application)
      end
  ...
  describe '#validate_rental_application' do
    ...
    it 'successfully validates a complete application' do
      VCR.use_cassette('validate_rental_application') do
        naborly_client.validate_rental_application(complete_application)
      end
  ...
  describe '#submit_rental_application' do
    it 'successfully submits a complete application' do
      VCR.use_cassette('submit_rental_application', record: :all) do
        expect {
          naborly_client.submit_rental_application(complete_application)
        }.to raise_error
      end
    ...
    it 'raises an error when submitting an incomplete application' do
      expect {
        VCR.use_cassette('submit_incomplete_application', record: :all) do
          naborly_client.submit_rental_application(incomplete_application)
        end
      }.to raise_error(Naborly::Ruby::BadRequestError, 'Code: 400')
    end
  end

  describe '#approve_deny_rental_application' do
    it 'successfully approves an application' do
      VCR.use_cassette('approve_application') do
        expect {
          naborly_client.approve_deny_rental_application({
            'applicationId' => application_id,
            'approve' => 'true'
          })
        }.to raise_error
    ...
    it 'raises an error when approving a non existent application' do
    end

    it 'successfully denies an application' do
    end

    it 'raises an error when denying an application without providding a deniedReason' do
    end

    it 'raises an error when denying an application without providing a valid deniedReason' do
    end

    it 'raises an error when denying an application with deniedReason "other" and not providing a deniedReasonDetail' do
    end
  end
end
